 ...
 ************************************************************************************/
package com.github.cabutchei.rsp.server.spi.runtimes;
...
import com.github.cabutchei.rsp.eclipse.core.runtime.SubMonitor;
...
public abstract class AbstractLicenseOnlyDownloadExecutor implements IDownloadRuntimeRunner {
	...
	private String initiateDownloadAndCreateServer(DownloadSingleRuntimeRequest req, 
		...
		final MessageContext<RSPWTPClient> client = ClientThreadLocal.getStore().getContext();
		IStatusRunnableWithProgress task = new IStatusRunnableWithProgress() {
			...
			public IStatus run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
				...
				final MessageContext<RSPWTPClient> prev = ClientThreadLocal.getStore().getContext();
	...
	protected com.github.cabutchei.rsp.api.dao.Status statusDao(int sev, String bundle, String msg) {
		return new com.github.cabutchei.rsp.api.dao.Status(sev, bundle, msg);
	}
